...
void handle(std::string ip)
{
  ...
  std::cout << ip <<"handler done" << '\n';
}

int main() {
    Proactor proactor;
    proactor.addHandler("one", [](){
      std::cout << "one handler called!" << '\n';
        // sleep(2);
        // std::cout << "one handler done" << '\n';
      std::async(std::launch::async, handle, "one");
    });
    proactor.addHandler("two", [](){
      std::cout << "two handler called!" << '\n';
        // sleep(4);
        // std::cout << "two handler done" << '\n';
      std::async(std::launch::async, handle, "two");
    });
...
/*
  main will start a handler and run an infinite while loop
  when an event occurs the event handler is triggered and calls async::launch::deffered
